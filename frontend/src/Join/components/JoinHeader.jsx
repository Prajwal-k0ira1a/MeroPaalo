...
export default function JoinHeader({ showTimer = false }) {
        ...
        <header className="w-full bg-white border-b border-slate-100 sticky top-0 z-50 transition-all duration-300">
            <div className="max-w-4xl mx-auto px-6 h-16 md:h-20 flex items-center justify-between">
                ...
                <Link to="/" className="flex items-center gap-2.5 group select-none">
                    <span className="w-8 h-8 rounded-lg bg-gradient-to-br from-teal-500 to-blue-600 flex items-center justify-center text-white font-bold text-base shadow-md shadow-teal-500/30 group-hover:shadow-teal-500/50 transition-all duration-300 group-hover:scale-105">
                        M
                    </span>
                ...
                <div className="md:hidden bg-white border-t border-slate-100 px-6 py-4 flex flex-col gap-2 animate-in slide-in-from-top duration-300">
                    {navLinks.map((link) => (
                        <Link
                            ...
                            className={`px-4 py-3 rounded-xl text-xs font-bold uppercase tracking-widest flex items-center justify-between group transition-all ${link.isActive
                                    ? "bg-teal-50 text-teal-600"
                                    : "text-slate-500 hover:bg-slate-50 hover:text-slate-700"
                                }`}
                        >
                            {link.label}
                            <svg className={`w-4 h-4 transition-transform group-hover:translate-x-1 ${link.isActive ? "text-teal-500" : "text-slate-300"}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3">
                    ...
                    {showTimer && (
                        <div className="mt-2 p-4 bg-slate-50 rounded-xl border border-slate-100 flex items-center justify-between sm:hidden">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-[10px] font-bold text-slate-600 uppercase tracking-widest">Auto-Refresh Sync</span>
                            </div>
                            <span className="text-xs font-black text-teal-600 tabular-nums bg-white px-2 py-0.5 rounded-lg border border-teal-100">{countdown}s</span>
                        </div>
                    )}
                </div>
            )}
...
function NavLink({ to, isActive, label }) {
    return (
        <Link
            to={to}
            className={`px-5 py-2 rounded-xl text-[11px] font-bold uppercase tracking-widest transition-all duration-300 ${isActive
...
function SyncIcon({ className }) {
    return (
        <svg className={className} width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3">
