  ...
  return (
    <div className="flex flex-col rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
      <div className="mb-5 flex items-center justify-between">
        <span className="text-base font-semibold text-slate-900">Upcoming Queue</span>
        ...
        {upcomingPatients.map((p) => (
          <div
            ...
            className={`flex items-center justify-between rounded-xl px-3 py-3 ${
              p.next ? "border border-teal-200 bg-teal-50" : "border border-transparent"
            }`}
          >
            <div>
            ...
            <div className="text-right">
              {p.next && <p className="text-xs font-semibold text-teal-700">NEXT</p>}
              <p className={`text-sm font-semibold ${p.next ? "text-teal-700" : "text-slate-700"}`}>{p.wait}</p>
